...
Cracker boxes (`cracker_box` USD asset) are picked from a 4×1 grid in the pick bin and placed onto thin green rectangle markers arranged in a 3×4 grid on the dropzone. The target grid is spaced widely enough along X (spacing_x=-0.18m) that three upright cracker boxes can sit comfortably side-by-side on adjacent column targets. Cracker boxes spawn upright (-90° X rotation). Pairing is sequential (default). Verification checks both marker placement (`is_on_top`) and vertical orientation (`is_vertical`). Uses the cortex-style BT (inherited verification config from `TableTaskCrackerBoxes1`).
