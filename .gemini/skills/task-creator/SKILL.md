...
8.  **Verify**: Run a mock version of the task with  `python run_mock_task.py --task <TaskClassName>`. If the task setup includes some randomization, then run the mock task multiple times with different seeds to verify that the task is robust to randomization.  If the mock task runs successfully, run the full task with `python run_task.py --task <TaskClassName>`.
