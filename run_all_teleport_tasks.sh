#!/usr/bin/env bash
# Run every simulation task (or a specified subset) with --teleport --auto-exit.
#
# Usage:
#   bash run_all_teleport_tasks.sh              # run all tasks, skipping SKIP_TASKS
#   bash run_all_teleport_tasks.sh 1 3 5        # run only task indices 1, 3, 5 (skip list ignored)
#
# Environment variable overrides:
#   NUM_RUNS    - number of runs per task           (default: 3)
...
#
# Examples:
#   NUM_RUNS=1 bash run_all_teleport_tasks.sh
#   SEEDS_FILE=seeds.json bash run_all_teleport_tasks.sh 1 3 5
#   SIM_LOGS_DIR=/tmp/run42 bash run_all_teleport_tasks.sh

DEFAULT_NUM_RUNS=3
DEFAULT_TASK_SET="--tasks2"
DEFAULT_SKIP_TASKS=("TableTask3Curobo" "TableTaskBottlesToConveyor2x")

MODE_LABEL="teleport"
MODE_TASK_ARGS="--teleport"
